...
*   **Missing Model File:** If `weights/best.pt` is missing, the script will show a clean error message and request placing the weights file instead of throwing an unhandled exception.
*   **Webcam Unavailable:** If your computer camera is blocked, in use, or unavailable, the system outputs: `✗ ERROR: Webcam is unavailable, disconnected, or blocked by another process.` and exits cleanly.
*   **Invalid / Corrupted Images:** The utility class validates image headers prior to running YOLO. If a file is corrupted or not a valid image format, it logs a warning: `✗ Error: Invalid or corrupted image file` and skips it.
*   **Empty Folders:** If running batch mode on an empty directory, it logs: `✗ Error: No valid images found in folder` and terminates cleanly.
*   **NumPy Version Conflicts:** PyTorch/YOLO may alert a warning concerning Numpy 2.x on older Python installations. The system contains defensive exceptions to prevent NumPy crashes on loading.
